...
Defines how `backup-git-repos run` and `backup-git-repos list` resolve each of their options across a command-line flag, an environment variable, and the YAML config file, and in what order one source wins over another.
