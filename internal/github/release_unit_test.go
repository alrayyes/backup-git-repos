...
const releaseAssetContent = "github release asset payload"

// releaseServer replies to the releases list with two releases -- one
// carrying a single uploaded asset whose API "url" points back at this same
// server, one carrying none -- and serves that asset's raw content when the
// request carries the "Accept: application/octet-stream" header
// downloadAsset always sets. This is what gives ReleaseExporter fast-lane
// (go test ./..., no build tag) coverage: the integration-tagged
// TestRecordedReleaseExporterContract covers the same exporter against
// hand-authored fixtures, but Codecov's patch-coverage gate only sees what
// runs without that tag.
	...
	srv = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprintf(w, `[
			{"tag_name":"v1.0.0","name":"v1.0.0","body":"release notes","author":{"login":"alice"},
			 ...
			 "assets":[{"name":"artifact.txt","size":%d,"url":%q}]},
...
func TestReleaseExporterUnit(t *testing.T) {
	...
	exp := github.NewReleaseExporter(client)
